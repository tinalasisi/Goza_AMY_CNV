...
df_saliva_final <- df_saliva %>% 
  ...
  select(TP_Before, TP_After, Primer2_Diploid, TP_Change, sAA_Change, sAA_Before, sAA_After, Flow_Rate_mean, Starch_Frequency, Citrus_Sum_120sec)
...
```{r fig-med-before, fig.cap="Mediator model for values before experiment}

plot_sem_model(fit_before)
...
# mutate(CNV_group = if_else(Primer2_Diploid > k, "High", "Low"))
